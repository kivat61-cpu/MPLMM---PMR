...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None):
    ...
    # angle_file = args.logs_path + '/Method-CE-grad-amp' + '/angle-' + args.dataset + '-' + args.fusion_method + '-bsz' + \
    #              str(args.batch_size) + '-lr' + str(args.learning_rate) \
    #              + '-epoch' + str(args.epochs) + '-' + args.modulation + str(args.alpha) + \
    #              '-ends' + str(args.modulation_ends) + '-' + str(args.num_frame) + '.txt'
    ...
    for step, batch_data in enumerate(dataloader):
        ...
        # 将 t_ids, t_mask, c_ids, c_mask 喂给模型
            ...
            if args.modulation_starts <= epoch <= args.modulation_ends:  # bug fixed
                ...
                if args.dataset == 'TextCode':
                    for name, parms in model.named_parameters():
                        ...
                        # 1. 匹配 'text' 分支，并确保梯度不为空
                        if 'text' in layer and parms.grad is not None and len(parms.grad.size()) >= 2:
                        ...
                        # 2. 匹配 'code' 分支，并确保梯度不为空
                        if 'code' in layer and parms.grad is not None and len(parms.grad.size()) >= 2:
                            if args.modulation == 'OGM_GE':  
...
def valid(args, model, device, dataloader):
    ...
    with torch.no_grad():
        ...
        #     else:
        #         a, v, out = model(spec, image)  # gray colored
        # 替换 train_epoch 和 valid 里的这个 for 循环开头
        for step, batch_data in enumerate(dataloader):
            t_ids = batch_data['text_input_ids'].to(device)
            t_mask = batch_data['text_attention_mask'].to(device)
            c_ids = batch_data['code_input_ids'].to(device)
            ...
            # 将 t_ids, t_mask, c_ids, c_mask 喂给模型
            a, v, out = model(t_ids, t_mask, c_ids, c_mask) # 故意用 a, v 接收，这样下方的 loss 计算代码就一行都不用改！
...
def main():
    ...
    if args.dataset == 'CGMNIST':
        ...
        # 当输入是 TextCode 时，调用咱们新写的文本-代码二分类器！
    ...
    # === 开始：您的专属数据 8:1:1 切分逻辑 ===
    print("正在读取并划分数据...")
    # 请确保您的文件与 main_CE_test_grad.py 在同级目录，或者写绝对路径
    master_df = pd.read_csv("jodatime.csv") 

    # 第一次切分：分离出 80% 训练集，20% 剩余集
    train_df, temp_df = train_test_split(
        master_df, test_size=0.2, random_state=42, stratify=master_df['label']
    )
    # 第二次切分：将剩余 20% 对半分为验证集和测试集
    ...
    if args.train:
        ...
        trainloss_file = args.logs_path + '/Method-CE-grad-amp' + '/train_loss-' + args.dataset + '-' + args.fusion_method + '-bsz' + \
                         str(args.batch_size) + '-lr' + str(args.learning_rate) \
                         + '-epoch' + str(args.epochs) + '-' + args.modulation + str(args.alpha) + \
                         '-ends' + str(args.modulation_ends) + '-' + str(args.num_frame) + '-optim-' + args.optimizer + '.txt'
        if not os.path.exists(args.logs_path + '/Method-CE-grad-amp'):
            os.makedirs(args.logs_path + '/Method-CE-grad-amp')

        save_path = args.ckpt_path + '/Method-CE-grad-amp' + '/model-' + args.dataset + '-' + args.fusion_method + '-bsz' + \
                    str(args.batch_size) + '-lr' + str(args.learning_rate) \
                    + '-epoch' + str(args.epochs) + '-' + args.modulation + str(args.alpha) + \
