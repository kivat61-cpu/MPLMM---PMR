...
class MULTModel(nn.Module):
    ...
    def get_network(self, self_type="l", layers=-1):
        ...
        if self_type in ["l", "al"]:
            ...
            embed_dim, attn_dropout = self.d_l, self.attn_dropout    # <--- 改回单倍维度
        elif self_type == "a_mem":
            embed_dim, attn_dropout = self.d_a, self.attn_dropout    # <--- 改回单倍维度
        else:
            raise ValueError("Unknown network type")
    ...
    def forward(self, x_l, x_a, missing_mod=None):
        x_l = F.dropout(
...
class PromptModel(nn.Module):
    def __init__(self, hyp_params):
        ...
        self.m_a = nn.Parameter(torch.zeros(self.alen, self.prompt_dim))

        self.m_l = nn.Parameter(torch.zeros(self.llen, self.prompt_dim))
    ...
    # def get_complete_data(self, x_l, x_a, x_v, missing_mode):
        ...
        # return x_l, x_a, x_v
    def get_complete_data(self, x_l, x_a, missing_mode):
        x_l, x_a = x_l.unsqueeze(dim=0), x_a.unsqueeze(dim=0)
        
        if missing_mode == 0:
            # 缺描述文本：用代码(a)生成文本(l)
            x_l = torch.cat(
                [self.generative_prompt[0, :, :], self.a2l(x_a)[0]], 
                dim=1
            ).unsqueeze(dim=0)
            x_l = self.l_ap(x_l.transpose(1, 2)).transpose(1, 2) + self.promptl_m
            x_a = self.proj_a(x_a) + self.prompta_nm
            ...
            # 缺代码：用描述文本(l)生成代码(a)
            x_a = torch.cat(
                [self.generative_prompt[1, :, :], self.l2a(x_l)[0]], 
                dim=1
            ).unsqueeze(dim=0)
            x_a = self.a_lp(x_a.transpose(1, 2)).transpose(1, 2) + self.prompta_m
            x_l = self.proj_l(x_l) + self.promptl_nm
    ...
    #     )
    def get_proj_matrix(self):
        # a_l: 都不缺失 (not missing)
        a_l = (
            self.prompta_nm @ self.m_a + 
            self.promptl_nm @ self.m_l
        ).unsqueeze(dim=0)
        
        # a_lm: 描述文本缺失 (l is missing)
        a_lm = (
            self.prompta_nm @ self.m_a + 
            self.promptl_m @ self.m_l
        ).unsqueeze(dim=0)
        
        # am_l: 代码缺失 (a is missing)
        am_l = (
            self.prompta_m @ self.m_a + 
            self.promptl_nm @ self.m_l
        ).unsqueeze(dim=0)
        
        # 对应 missing_mode = 0(缺文本), 1(缺代码), 2(都不缺)
        self.mp = torch.cat(
            [a_lm, am_l, a_l], dim=0
        )

    # def forward(self, x_l, x_a, x_v, missing_mod):
    def forward(self, x_l, x_a, missing_mod):
        x_l = F.dropout(
            x_l.transpose(1, 2), p=self.embed_dropout, training=self.training
        )
        x_a = x_a.transpose(1, 2)
        ...
        for idx in range(len(x_l)):
            ...
            x_l_temp, x_a_temp, = self.get_complete_data(
                # x_l[idx], x_a[idx], x_v[idx], missing_mod[idx]
                x_l[idx], x_a[idx], missing_mod[idx]
            )
            if xx_l is None:
                xx_l = x_l_temp
                xx_a = x_a_temp
        ...
        h_ls = torch.cat([h_l_with_as, batch_prompt[0].transpose(0,1)], dim=0)
        h_ls = self.trans_l_mem(h_ls)
        if type(h_ls) == tuple:
            h_ls = h_ls[0]
        ...
        h_as = torch.cat([h_a_with_ls, batch_prompt[1].transpose(0,1)], dim=0)
        h_as = self.trans_a_mem(h_as)
        if type(h_as) == tuple:
            h_as = h_as[0]
